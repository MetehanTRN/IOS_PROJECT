...
export default function BlackListModal({ visible, onClose, onSelectToAdd }: Props) {
  ...
  const removeFromBlacklist = async (id: string) => {
    Alert.alert("Emin misiniz?", "Bu plakayı kara listeden çıkarmak istiyor musunuz?", [
  ...
  const confirmAddToPlates = (plate: string) => {
    Alert.alert("Plakayı Kaydet", `${plate} plakalı aracı kayıtlı plaka olarak eklemek ister misiniz?`, [
      { text: "İptal", style: "cancel" },
      {
        text: "Evet",
        onPress: () => {
          onClose(); // modalı kapat
          onSelectToAdd(plate); // add modal'ı tetikle
        },
      },
    ]);
  };
...
const styles = StyleSheet.create({
  ...
  plate: { fontSize: 16 },
  addBtn: { backgroundColor: "#2ecc71", padding: 8, borderRadius: 6 },
  delBtn: { backgroundColor: "#e74c3c", padding: 8, borderRadius: 6 },
